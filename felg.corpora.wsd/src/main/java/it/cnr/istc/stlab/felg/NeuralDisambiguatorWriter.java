...
public class NeuralDisambiguatorWriter /* extends DisambiguatorContextSentenceBatch implements AutoCloseable */ {
	...
	public NeuralDisambiguatorWriter(String pythonPath, String neuralPath, String weightsPath) {
		this(pythonPath, neuralPath, Collections.singletonList(weightsPath));
	}

	public NeuralDisambiguatorWriter(String pythonPath, String neuralPath, String weightsPath, boolean clearText,
			int batchSize) {
	...
	public NeuralDisambiguatorWriter(String pythonPath, String neuralPath, List<String> weightsPath) {
		this(pythonPath, neuralPath, weightsPath, false, 1);
	}

	public NeuralDisambiguatorWriter(String pythonPath, String neuralPath, List<String> weightsPaths, boolean clearText,
			int batchSize) {
...
//		super(batchSize);
	...
	private void writePredictInput(List<Sentence> sentences, OutputStream os) throws Exception {
		for (Sentence sentence : sentences) {
			List<Word> words = sentence.getWords();
			os.write(JsonStream.serialize(new it.cnr.istc.stlab.felg.model.Sentence(sentence)).getBytes());
			os.write('\n');
			writePredictInputSampleX(words, os);
			writePredictInputSampleZ(words, os);
		}
		os.flush();
	}

	private void writePredictInputSampleX(List<Word> words, OutputStream os) throws Exception {
		for (Word w : words) {
			if (lowercaseWords) {
			...
			os.write((StringUtils.join(featureValues, "/") + " ").getBytes());
		}
		os.write('\n');
	}

	private void writePredictInputSampleZ(List<Word> words, OutputStream os) throws Exception {
		if (outputFeatures <= 0)
			return;
		if (extraLemma)
			return;
				...
				if (!possibleSenseKeys.isEmpty()) {
					...
					for (String possibleSenseKey : possibleSenseKeys) {
						...
						if (outputVocabulary.get(senseFeatureIndex).containsKey(possibleSynsetKey)) {
							possibleSenseKeyIndices
									.add("" + outputVocabulary.get(senseFeatureIndex).get(possibleSynsetKey));
						}
					}
					if (possibleSenseKeyIndices.isEmpty()) {
						os.write("0 ".getBytes());
					} else {
						os.write((StringUtils.join(possibleSenseKeyIndices, ";") + " ").getBytes());
					}
				} else {
					os.write("0 ".getBytes());
				}
...
//	private void disambiguateNoCatch(List<Sentence> sentences, String senseTag, String confidenceTag, OutputStream os,
//			BufferedReader br) throws Exception {
//		writePredictInput(sentences, os);
...
//		try {// FIXME
//			disambiguateNoCatch(sentences, senseTag, confidenceTag, null, null);
//		} catch (Exception e) {
//			throw new RuntimeException(e);
//		}
//	}

	public void writeOnOutputStream(List<Sentence> sentences, OutputStream os) throws Exception {
		writePredictInput(sentences, os);
	}
